...
export default function RandevularimClient({ appointments: initialAppointments }: RandevularimClientProps) {
  ...
  function getStatusBadge(status: string) {
    const badges = {
      ...
      cancelled_by_student: { text: 'İptal Edildi (Öğrenci)', class: 'bg-red-100 text-red-800' },
      cancelled_by_teacher: { text: 'İptal Edildi (Öğretmen)', class: 'bg-orange-100 text-orange-800' }
    }
    ...
    <div>
      ...
      {filteredAppointments.length === 0 ? (
        <div className="bg-white shadow rounded-lg p-12 text-center">
          ...
          <p className="text-gray-600 mb-6">{filter === 'all' ? 'Henüz hiç randevu oluşturmadınız.' : 'Bu filtrede gösterilecek randevu bulunamadı.'}</p>
          <a href="/ogrenci/randevu-al" className="inline-block bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors">
            Randevu Al
          </a>
        </div>
      ) : (
            ...
            <div key={apt.id} className="bg-white shadow rounded-lg p-6 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between">
                ...
                <div className="flex flex-col gap-2 ml-4">
                  <button onClick={() => setSelectedAppointment(apt)} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium">
                    Detay
                  </button>
                  {apt.status === 'scheduled' && (
                    <button onClick={() => handleCancel(apt.id)} disabled={loading} className="px-4 py-2 bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition-colors text-sm font-medium disabled:opacity-50">
                      İptal Et
                    </button>
                  )}
                </div>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-lg text-gray-900">{selectedAppointment.teacher.profiles.full_name}</p>
              </div>
              <div>
              ...
              {selectedAppointment.notes && (
                <div>
                  ...
                  <p className="text-gray-900 bg-gray-50 p-3 rounded-lg mt-1">{selectedAppointment.notes}</p>
                </div>
              )}
              <div>
            ...
            <div className="mt-6 flex gap-3">
              {selectedAppointment.status === 'scheduled' && (
                <button onClick={() => handleCancel(selectedAppointment.id)} disabled={loading} className="flex-1 bg-red-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-red-700 transition-colors disabled:opacity-50">
                  Randevuyu İptal Et
                </button>
              )}
